...
def test117():
    ...
    print('test117:进入批发→组→采购管理→采购进货')
    time.sleep(time1)

    # 点击采购入库
    driver.find_element_by_link_text('采购进货').click()
    time.sleep(time2)
    ...
    # 输入数量
    driver.find_element_by_xpath(
        '/html/body/div[8]/div/div[2]/div/div[2]/div[2]/div[1]/div/div/div[2]/div/div/div/div/div[2]/div/div/div[1]/div/input').send_keys(discount)
    ...
    try:
        ...
        time.sleep(time1)
    print(shop_name + ':采购入库' + discount + '件下单成功', '\n采购入库下单正常')
    driver.quit()
